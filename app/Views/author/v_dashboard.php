...
<body class="page-header-fixed compact-menu pace-done page-sidebar-fixed">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Pastikan variabel PHP $total_pemasukan dan $total_pengeluaran tersedia dan berisi nilai numerik
            const labelsPerbandingan = ['Pemasukan', 'Pengeluaran'];
            const dataPerbandingan = [<?= $total_pemasukan ?? 0; ?>, <?= $total_pengeluaran ?? 0; ?>]; // Tambahkan null coalescing untuk keamanan

            const ctxPerbandingan = document.getElementById('chart-perbandingan');
            if (ctxPerbandingan) { // Pastikan elemen canvas ada
            ...
            // Chart Visitors This Month (DISAMAKAN DENGAN CHART.JS MODERN)
            var ctxVisitors = document.getElementById("canvas");
            if (ctxVisitors) { // Pastikan elemen canvas ada
                new Chart(ctxVisitors.getContext("2d"), {
                    type: 'line', // Tipe chart sebagai 'line'
                    data: {
                        // Pastikan $month dan $value adalah array JSON valid dari PHP.
                        // Contoh: $month = "['Jan', 'Feb', 'Mar']" dan $value = "[100, 150, 120]"
                        labels: <?= $month ?? '[]'; ?>, // Tambahkan null coalescing untuk keamanan
                        datasets: [
                            {
            ...
            // Kode untuk chartKas (jika Anda memiliki chart ini di tempat lain)
            // Bagian ini dikomentari karena tidak ada elemen <canvas id="chartKas"></canvas> di HTML yang Anda berikan.
            // Jika Anda ingin menambahkan chart ini, pastikan untuk menambahkan elemen canvas yang sesuai di HTML.
